...
<head>
  ...
  <title>Asian Movies | Moviesarea.online</title>
  <meta name="description" content="Asian Movies free download site with fast server">
  <meta name="keywords" content="Asian Movies, free download, movies, free movies, download, download movies, moviesarea, moviesarea.online, hdmovies,all movies, whach online movies, whach online">
...
<body>
<div class="tital">Asian Movies download | Moviesarea</div> 
  ...
  <div class="container-fluid" style="margin-top: 70px;padding-right: 0;padding-left: 0;">
    ...
    <div class="wrap">
      ...
      $q1 = mysqli_query($link, "SELECT * FROM movies_data WHERE asian_movies = 'yes'");
      ...
      $sql = "SELECT * FROM movies_data WHERE asian_movies = 'yes' ORDER BY id DESC LIMIT ".$p.", ".$rowsperpage;
      ...
      if($_REQUEST['page'] > 1){
        ...
        $prev_page_main = '<div class=""><a href= "asian_movies.php?page='.$prev_page.'">Previous</a></div>';   

      }else{
        $prev_page = $_REQUEST['page'] - 1;
        $prev_page_main = '<div class=" disabled"><a href= "asian_movies.php?page='.$prev_page.'">Previous</a></div>';   
      }
      ...
      if($check < $count){
        ...
        $next_page_main = '<div class=""><a href= "asian_movies.php?page='.$next_page.'">next</a></div>';
      }else{
        $next_page = $_REQUEST['page'] + 1;
        $next_page_main = '<div class=" disabled"><a href= "asian_movies.php?page='.$next_page.'">next</a></div>'; 
      }
      ...
      <div class="container">
        <div class="pagination" style="margin-top:50px; width: 100%">
          ...
          for ($i=1; $i<=$limit; $i++) { 
            if ($i==$_REQUEST['page']) {
             $page_number = $i;
             echo'<a href="asian_movies.php?page='.$page_number.'" class="active disabled">'.$page_number.'</a>';
           }else{
            echo'<a href="asian_movies.php?page='.$i.'">'.$i.'</a>';
          }
        }
        echo $next_page_main;
        ?>
